<template>
  <div class="container-xl mt-5 pt-4 px-3">
    ...
    <div class="mt-4 d-flex flex-column align-items-center">
      ...
      <img :src="imagenPerfil || imagenPorDefecto" :alt="t('userData.emailLabel')" class="rounded-circle"
      ...
      <p v-if="profesor?.usuario"><strong>{{ t('userData.emailLabel') }}</strong> {{ profesor.usuario.email }}</p>
    </div>

    <!-- Botones -->
    <div class="mt-5 text-center " v-if="profesor">
      <button class="btn btn-secondary me-2" @click="mostrarFormulario('edit')" v-if="profesor?.usuario">
        {{ t('userData.editUser') }}
      </button>
      <button class="btn btn-danger" @click="eliminarUsuario" v-if="profesor?.usuario">
        {{ t('userData.deleteUser') }}
      </button>
      <div v-else>
        <button class="btn btn-primary me-2" @click="mostrarFormulario('create')">
          {{ t('userData.createUser') }}
        </button>
      </div>
    </div>
    ...
    <div class="mt-5">
      ...
      <div class="text-center mt-4 mb-3">
        <button class="btn btn-outline-warning" @click="mostrarFormularioAusencia = !mostrarFormularioAusencia">
          {{ mostrarFormularioAusencia ? t('userData.cancel') : t('userData.createAbsence') }}
        </button>
      </div>

      <FormularioCrearAusencia v-if="mostrarFormularioAusencia" :idProfesor="idProfesorEfectivo"
        @ausenciaCreada="onAusenciaCreada" @error="mensaje => mostrarModal(t('modal.importErrorTitle'), mensaje, 'error')" />
...
async function obtenerDatosProfesor() {
  try {
    ...
    mostrarModal(t('modal.importErrorTitle'), t('userData.loadingError'), 'error')
  }
}
...
function confirmarYSubir() {
  if (confirm(t('userData.profilePhotoConfirm'))) {
...
async function subirImagen(event) {
  ...
  try {
    ...
    mostrarModal(t('userData.userModified'), response.data, 'success')
    await obtenerDatosProfesor()
  } catch (error) {
    const mensajeError = error.response?.data || t('userData.unknownError')
    mostrarModal(t('modal.importErrorTitle'), mensajeError, 'error')
  }
}

async function guardarUsuario(datosFormulario) {
  const { idProfesor: idProf, email, password, rol, nombre } = datosFormulario

  if (!email || !password || !rol || !nombre || !idProf) {
    mostrarModal(t('modal.importErrorTitle'), t('userData.userFieldIncomplete'), 'warning')
    return
  }

  const payload = { idProfesor: idProf, nombre, email, contraseña: password, rol }
  ...
  try {
    ...
    mostrarModal(t('userData.userCreated'), `${t('userData.userCreatedMessage')} ${nombre}`, 'success')
    formularioActivo.value = null
    await obtenerDatosProfesor()
  } catch (error) {
    if (error.response?.status === 400 && error.response.data) {
      erroresFormulario.value = error.response.data
      const mensaje = error.response.data.mensaje || t('userData.unknownError')
      mostrarModal(t('modal.importErrorTitle'), mensaje, 'error')
    } else {
      mostrarModal(t('modal.importErrorTitle'), t('userData.unknownError'), 'error')
    }
  } finally {
...
async function modificarUsuario(datosFormulario) {
  ...
  try {
    ...
    mostrarModal(t('userData.userModified'), `${t('userData.userModifiedMessage')} ${nombre}`, 'success')
    formularioActivo.value = null
    await obtenerDatosProfesor()
  } catch (error) {
    if (error.response?.status === 400 && error.response.data) {
      ...
      const mensaje = error.response?.data || t('userData.unknownError')
      mostrarModal(t('modal.importErrorTitle'), mensaje, 'error')
    }
  } finally {
    isLoading.value = false
...
async function eliminarUsuario() {
  ...
  const confirmado = confirm(`${t('userData.confirmDeleteUser')} ${nombre}?`)
  ...
  try {
    ...
    mostrarModal(t('modal.importErrorTitle'), t('userData.userDeleteFailed'), 'error')
  } finally {
    isLoading.value = false
  }
}
...
function onAusenciaCreada() {
  mostrarModal(t('userData.absenceCreated'), t('userData.absenceCreatedMessage'), 'success')
  mostrarFormularioAusencia.value = false
  ausenciasKey.value = Date.now()
}
